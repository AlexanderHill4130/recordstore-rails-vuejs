<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addRecord">
      ...
      <input type="submit" value="Add Record" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-400 hover:bg-green-800 block w-full py-4 text-white items-center justify-center">
    </form>
    ...
    <ul class="list-reset mt-4">
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        ...
        <button class="bg-transparent text-sm hover:bg-blue-800 hover:text-white text-blue-400 border border-blue-400 no-underline font-bold py-2 px-4 mr-2 rounded"
          @click.prevent="editRecord(record)">Edit</button>

        <button class="bg-transparent text-sm hover:bg-red-800 text-red-400 hover:text-white no-underline font-bold py-2 px-4 rounded border border-red"
        ...
        <div v-if="record == editedRecord">
          <form action="" @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded border border-gray-400 mt-4">
              ...
              <input type="submit" value="Update" class="bg-transparent text-sm hover:bg-blue-800 hover:text-white text-blue-400 border border-blue-400 no-underline font-bold py-2 px-4 mr-2 rounded">
            </div>
          </form>
        </div>
      </li>
